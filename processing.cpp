    ...
    // if (width >= 2) {
        for (int row = 1; row < Matrix_height(energy); row++) {
            for (int col = 0; col < width; col++) {
                if (col == 0) {
                    min_val = Matrix_min_value_in_row(cost, row - 1, 0, 2);
                    *Matrix_at(cost, row, col) =
                        *Matrix_at(energy, row, col) + min_val;
                }
                else if (col == width - 1) {
                    min_val = Matrix_min_value_in_row(cost, row - 1,
                                                      width - 2, width);
                        *Matrix_at(cost, row, col) =
                        *Matrix_at(energy, row, col) + min_val;
                }
                else {
                    min_val = Matrix_min_value_in_row(cost, row - 1, col - 1, col + 2);
                    *Matrix_at(cost, row, col) =
                        *Matrix_at(energy, row, col) + min_val;
                }
            }
...
void remove_vertical_seam(Image *img, const int seam[]) {
    ...
    Pixel* copy = new Pixel;

    //copies left side of seam into image named small
    for (int row = 0; row < Image_height(img); row++) {
        for (int col = 0; col < seam[row]; col++) {
            *copy = Image_get_pixel(img, row, col);
            Image_set_pixel(small, row, col, *copy);
        }
    }
    //copies right side of seam
    for (int row = 0; row < Image_height(img); row++) {
        for (int col = seam[row] ; col < Image_width(small); col++) {
            *copy = Image_get_pixel(img, row, col + 1);
            Image_set_pixel(small, row, col, *copy);
...
void seam_carve_width(Image *img, int newWidth) {
    int width_count = Image_width(img);
    Matrix* energy = new Matrix;
    Matrix* cost = new Matrix;
    int seam[MAX_MATRIX_HEIGHT];

    while (width_count > newWidth) {
